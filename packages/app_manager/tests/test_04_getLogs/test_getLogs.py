...
class TestManagerAppGetLogs:
    ...
    async def test_should_throw_error_when_device_returns_invalid_data(self, connection, test_case):
        mock_sdk = MagicMock()
        error_class = DeviceAppError if test_case['errorInstance'] == 'DeviceAppError' else GetLogsError
        error_instance = error_class(DeviceAppErrorType.INVALID_MSG_FROM_DEVICE if error_class == DeviceAppError else GetLogsErrorType.LOGS_DISABLED)
    ...
    async def test_should_throw_error_when_device_returns_error(self, connection, test_case):
        mock_sdk = MagicMock()
        error_class = DeviceAppError if test_case['errorInstance'] == 'DeviceAppError' else GetLogsError
        error_instance = error_class(DeviceAppErrorType.USER_REJECTION if error_class == DeviceAppError else GetLogsErrorType.LOGS_DISABLED)
        mock_sdk.run_operation = AsyncMock(side_effect=error_instance)
        mock_sdk.destroy = AsyncMock()
