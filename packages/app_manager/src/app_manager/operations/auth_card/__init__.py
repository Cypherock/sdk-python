...
async def verify_serial_signature(
    ...
    # Send initiate query
    query = {
        "initiate": {"card_index": card_number, "is_pair_required": is_pair_required}
    }
    ...
    challenge = await card_auth_service.verify_card_serial_signature(
        **verification_params
    )
    serial = result.serial_signature.serial
...
async def verify_challenge_signature(
    ...
    verification_result = await card_auth_service.verify_card_challenge_signature(
        **verification_params
    )
...
async def auth_card(
    sdk: ISDK, params: Optional[IAuthCardParams] = None
) -> Dict[str, str]:
    """
    ...
    if params and params.card_number is not None:
        assert_condition(
            1 <= params.card_number <= 4, "Card number should be one of 1,2,3,4"
        )
