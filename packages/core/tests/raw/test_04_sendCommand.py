...
from packages.core.tests.raw.__fixtures__.sendCommand import raw_send_command_test_cases
...
async def setup_teardown_sdk():
    ...
    with patch("datetime.now", return_value=raw_send_command_test_cases["constantDate"]):
...
@pytest.mark.asyncio
async def test_should_be_able_to_send_command(setup_teardown_sdk):
    ...
    for test_case in raw_send_command_test_cases["valid"]:
        ...
        await sdk.deprecated.send_command(
            command_type=test_case["commandType"],
            data=test_case["data"],
...
async def test_should_throw_error_with_invalid_arguments(setup_teardown_sdk):
    sdk, connection = setup_teardown_sdk
    for test_case in raw_send_command_test_cases["invalidArgs"]:
        ...
        assert e.value.message == "Invalid arguments provided to sendCommand"
