...
class TestBtcAppGetXpubs:
    """Test BtcApp.getXpubs functionality."""
    ...
    @pytest.mark.asyncio
    async def test_should_be_able_to_get_xpub(self, btc_app):
        """Test valid getXpubs operations."""
        for test_case in fixtures.valid:
            # Setup mocks for this test case
            on_event = setup_mocks(test_case)
            
            # Execute getXpubs with test parameters
            output = await btc_app.get_xpubs(
                **test_case.params,
                on_event=on_event,
            )
    ...
    async def test_should_throw_error_with_invalid_arguments(self, btc_app):
        """Test getXpubs with invalid arguments."""
        for test_case in fixtures.invalid_args:
            ...
            with pytest.raises(test_case.error_instance):
                await btc_app.get_xpubs(test_case.params)
    ...
    async def test_should_throw_error_when_device_returns_invalid_data(self, btc_app):
        """Test getXpubs when device returns invalid data."""
    ...
    async def test_should_throw_error_when_device_returns_error(self, btc_app):
        """Test getXpubs when device returns errors."""
...
@pytest.mark.asyncio
async def test_get_xpubs_valid_cases(mock_btc_app):
    """Standalone test for valid getXpubs cases."""
    for test_case in fixtures.valid:
        on_event = setup_mocks(test_case)
        
        output = await mock_btc_app.get_xpubs(
            **test_case.params,
            on_event=on_event,
        )
...
@pytest.mark.asyncio
async def test_get_xpubs_invalid_args(mock_btc_app):
    """Standalone test for invalid argument cases."""
    for test_case in fixtures.invalid_args:
        setup_mocks(test_case)
        
        with pytest.raises(test_case.error_instance):
            await mock_btc_app.get_xpubs(test_case.params)


@pytest.mark.asyncio
async def test_get_xpubs_invalid_data(mock_btc_app):
...
@pytest.mark.asyncio
async def test_get_xpubs_device_errors(mock_btc_app):
...
__all__ = [
    'TestBtcAppGetXpubs',
    'test_get_xpubs_valid_cases',
    'test_get_xpubs_invalid_args',
    'test_get_xpubs_invalid_data',
    'test_get_xpubs_device_errors',
    'mock_btc_app',
]
