...
class TestBtcAppSignTxn:
    """Test BtcApp.signTxn functionality."""
    ...
    @pytest.mark.asyncio
    async def test_should_be_able_to_sign_txn(self, btc_app):
        """Test valid signTxn operations."""
        for test_case in fixtures.valid:
            # Setup mocks for this test case
            on_event = setup_mocks(test_case)
            
            # Execute signTxn with test parameters
            output = await btc_app.sign_txn(
                **test_case.params,
                on_event=on_event,
            )
    ...
    async def test_should_throw_error_with_invalid_arguments(self, btc_app):
        """Test signTxn with invalid arguments."""
        for test_case in fixtures.invalid_args:
            ...
            with pytest.raises(test_case.error_instance):
                await btc_app.sign_txn(test_case.params)
    ...
    async def test_should_throw_error_when_device_returns_invalid_data(self, btc_app):
        """Test signTxn when device returns invalid data."""
    ...
    async def test_should_throw_error_when_device_returns_error(self, btc_app):
        """Test signTxn when device returns errors."""
...
@pytest.mark.asyncio
async def test_sign_txn_valid_cases(mock_btc_app):
    """Standalone test for valid signTxn cases."""
    for test_case in fixtures.valid:
        on_event = setup_mocks(test_case)
        
        output = await mock_btc_app.sign_txn(
            **test_case.params,
            on_event=on_event,
        )
...
@pytest.mark.asyncio
async def test_sign_txn_invalid_args(mock_btc_app):
    """Standalone test for invalid argument cases."""
    for test_case in fixtures.invalid_args:
        setup_mocks(test_case)
        
        with pytest.raises(test_case.error_instance):
            await mock_btc_app.sign_txn(test_case.params)


@pytest.mark.asyncio
async def test_sign_txn_invalid_data(mock_btc_app):
...
@pytest.mark.asyncio
async def test_sign_txn_device_errors(mock_btc_app):
...
__all__ = [
    'TestBtcAppSignTxn',
    'test_sign_txn_valid_cases',
    'test_sign_txn_invalid_args',
    'test_sign_txn_invalid_data',
    'test_sign_txn_device_errors',
    'mock_btc_app',
]
